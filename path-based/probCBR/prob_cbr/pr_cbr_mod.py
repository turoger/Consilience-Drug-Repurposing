...
import torch
import wandb
from clustering.entity_clustering import cluster_entities
...
from data.get_paths import get_paths
...
# Init parser first, then call main
...
parser.add_argument(
    "--only_preprocess",
    action="store_true",
    help="If on, only calculates subbgraph, prior map and precision map",
)
# Clustering args
...
# Ranking method args, not implemented yet
# parser.add_argument("--aggr_type1", type=str, default="none", help="none/sum")
# parser.add_argument(
#     "--aggr_type2", type=str, default="sum", help="sum/max/noisy_or/logsumexp"
# )
...
args.output_dir = os.path.join(
    args.expt_dir, "data", "outputs", args.dataset_name, args.name_of_run
)
if not os.path.exists(args.output_dir):
    os.makedirs(args.output_dir)
logger.info(f"Output directory: {args.output_dir}")

# argparse can't handle bools, so convert them from numbers
...
class ProbCBR(object):
    ...
    @staticmethod
    def calc_sim(
        adj_mat: torch.Tensor, query_entities: torch.LongTensor
    ) -> torch.Tensor:
        """
        :param adj_mat: N X R
    ...
    def get_nearest_neighbor_naive(
        self, e1: str, r: str, k: Optional[int] = 5
    ) -> List[str]:
        """
    ...
    def rank_programs(self, list_programs: List[List[str]], r: str) -> List[List[str]]:
        ...
        for p in unique_programs:
            try:
                path_and_scores.append(
                    (
                        p,
                        args.path_prior_map_per_relation[self.c][r][p]
                        * args.precision_map[self.c][r][p],
                    )
                )
            except KeyError:
                # TODO: Fix key error
                if len(p) == 1 and p[0] == r:
                    ...
                    #     c = 0
                    #     score = (
                    #         args.path_prior_map_per_relation_fallback[c][r][p]
                    #         * args.precision_map_fallback[c][r][p]
                    #     )
        ...
        # sort wrt counts
        sorted_programs = [
            k for k, v in sorted(path_and_scores, key=lambda item: -item[1])
        ]
    ...
    def execute_programs(
        ...
        for path in path_list:
            if executed_path_counter == args.max_num_programs:
                break
            ans = self.execute_one_program(e, path, depth=0, max_branch=max_branch)
            temp = []
            if args.use_path_counts:
                try:
                    if (
                        path in args.path_prior_map_per_relation[self.c][r]
                        and path in args.precision_map[self.c][r]
                    ):
                        path_score = (
                            args.path_prior_map_per_relation[self.c][r][path]
                            * args.precision_map[self.c][r][path]
                        )
                    else:
                        # no fall back
                        path_score = 0
    ...
    def rank_answers(
        ...
        def rank_entities_by_max_score(score_map):
            """
            sort entities with respect to the top value.
            If there are multiple entities with the same score, then sort with respec to the second
            """
    ...
    def get_hits(
        ...
        rr = 0.0
        (e1, r) = query
        all_gold_answers = args.all_kg_map[(e1, r)]
    ...
    def do_symbolic_case_based_reasoning(self):
        ...
        for ex_ctr, ((e1, r), e2_list) in enumerate(tqdm(self.eval_map.items())):
            ...
            self.c = args.cluster_assignments[self.entity_vocab[e1]]
            all_programs = self.get_programs_from_nearest_neighbors(
                e1, r, self.get_nearest_neighbor_inner_product, num_nn=args.k_adj
            )
    ...
    def calc_precision_map(self, output_filenm=""):
        ...
        train_map = [
            ((e1, r), e2_list) for ((e1, r), e2_list) in self.train_map.items()
        ]
        for (e1, r), e2_list in tqdm(train_map):
            c = args.cluster_assignments[self.entity_vocab[e1]]
    ...
    def calc_prior_path_prob(self, output_filenm=""):
        ...
        unique_cluster_ids = (
            set()
        )  # have to do this since the assigned cluster ids doesnt seems to be contiguous or start from 0 or end at K-1
        for c in args.cluster_assignments:
            unique_cluster_ids.add(c)
        for c in unique_cluster_ids:
            for _, ((e1, r), e2_list) in enumerate(tqdm((self.train_map.items()))):
                if args.cluster_assignments[self.entity_vocab[e1]] != c:
...
def main(args):
    ...
    # Generate a subgraph if it doesn't exist at the location specified or no name is given
    if args.subgraph_file_name == "":
        args.subgraph_file_name = (
            f"paths_{args.num_paths_to_collect}_{args.max_path_len}hop"
        )
    ...
    l2norm[0] += np.finfo(
        float
    ).eps  # to encounter zero values. These 2 indx are PAD / NULL
    ...
    all_kg_map = load_data_all_triples(
        args.train_file, args.dev_file, os.path.join(data_dir, "test.txt")
    )
    args.all_kg_map = all_kg_map

    prob_cbr_agent = ProbCBR(
        args,
        full_map,
        train_map,
        eval_map,
    ...
    bck_dir_name = os.path.join(
        args.expt_dir, "data", "outputs", args.dataset_name, f"linkage={args.linkage}"
    )
    if not os.path.exists(bck_dir_name):
        os.makedirs(bck_dir_name)

    if args.linkage > 0:
        if os.path.exists(os.path.join(bck_dir_name, "cluster_assignments.pkl")):
            logger.info(
                f"Clustering with linkage {args.linkage} found, loading them...."
            )

            with open(
                os.path.join(bck_dir_name, "cluster_assignments.pkl"), "rb"
            ) as fin:
                args.cluster_assignments = pickle.load(fin)

        else:
            logger.info(f"Clustering entities with linkage = {args.linkage}...")
            args.cluster_assignments = cluster_entities(adj_mat, args.linkage)
            logger.info(
                f"There are {np.unique(args.cluster_assignments).shape[0]} unique clusters"
            )

            logger.info(f"Dumping cluster assignments of entities at {bck_dir_name}")

            with open(
                os.path.join(bck_dir_name, "cluster_assignments.pkl"), "wb"
            ) as fout:
                pickle.dump(args.cluster_assignments, fout)

    else:
        # when linkage is 0, all entities are in one cluster
        args.cluster_assignments = np.zeros(adj_mat.shape[0])
        logger.info("linkage is 0, all entities are in one cluster")
        logger.info(f"Dumping cluster assignments of entities at {bck_dir_name}")

        with open(os.path.join(bck_dir_name, "cluster_assignments.pkl"), "wb") as fout:
            pickle.dump(args.cluster_assignments, fout)
    ...
    if not os.path.exists(path_prior_map_filenm):
        logger.info("Path prior weights don't exist. Calculating path prior weights.")
        args.path_prior_map_per_relation = prob_cbr_agent.calc_prior_path_prob(
            output_filenm=path_prior_map_filenm
        )
    ...
    # path_prior_map_filenm = os.path.join(
    #     bck_dir_name, "path_prior_map.pkl"
    # )
    # if not os.path.exists(path_prior_map_filenm):
    ...
    # hard coded linkage again
    # args.linkage = linkage_bck
    # args.cluster_assignments = cluster_assignments_bck

    precision_map_filenm = os.path.join(bck_dir_name, "precision_map.pkl")
    if not os.path.exists(precision_map_filenm):
        logger.info("Precision map doesn't exist. Calculating precision map.")
    ...
    if not args.only_preprocess:
        logger.info("Starting symbolic case based reasoning")
